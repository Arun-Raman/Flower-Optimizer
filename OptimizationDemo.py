...
#---------------------EVERYTHING BELOW IS THE GUROBI PORTION (AND CAN THUS BE REPLACED WITH SOME OTHER SOFTWARE IF NEED BE)! -------------------------



model=gp.Model()   #<-we made a model called "model."
...
x=model.addVars([flower["Identifier"] for flower in filteredlist], vtype=GRB.INTEGER, lb=0, ub=15, name="x")
#To be more thorough, we can do something like 0 to infinity, but here I've done from 0 to 15 in order to decrease computing power. This can be changed as needed using the following line:
# x=model.addVars([flower["Identifier"] for flower in filteredlist], vtype=GRB.INTEGER, lb=0, ub=GRB.INFINITY, name="x")
...
#These need to be two separate constraints, and quicksum needs to be used
model.addConstr((quicksum((x[flower["Identifier"]] * flower["Number of Flowers per Package"]) for flower in filteredlist)) <= amount_high, name="Constraint 1")
model.addConstr((quicksum((x[flower["Identifier"]] * flower["Number of Flowers per Package"]) for flower in filteredlist)) >= amount_low, name="Constraint 2")
...
model.setObjective((quicksum((x[flower["Identifier"]] * flower["Cost"]) for flower in filteredlist))) ### putting in the objective function

model.update() #update the model to take in all the changes
...
#GRB.OPTIMAL means there was a procurement option that fits whatever constraints Paula wants, while GRB.INF_OR_UNBD means, for all intents and purposes, that there isn't
if (model.Status == GRB.OPTIMAL):
    for i in x:
        #For post-processing, Gurobi requires you to use the solution value, x[i].X, not just the Gurobi variable at face value (like x[i])
...
model.dispose() #dispose the model, you should do this after every run
